...
class FastaLexer(RegexLexer):
    name = "FASTA"
    aliases = ["fasta"]
    filenames = ["*.fasta", "*.fa", "*.fna", "*.faa"]
    reserved = [">"]
    tokens = {
        "root": [
            (r"^(>)(.*)$", bygroups(Operator.Word, Comment)),
            (r"^[^>].*$", Name)
        ]
    }

class NewickLexer(RegexLexer):
    name = "Newick"
    aliases = ["newick"]
    filenames = ["*.tre"]
    tokens = {
        "root": [
            (r'"[^"]*"', Comment),
            (r"(:)([0-9.]*)", bygroups(Operator.Word, Number.Float)),
            (r"[)(,;]", Operator.Word)
        ]
    }
